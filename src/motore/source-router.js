...
function statoErroreGestito(errore, timeout) {
  ...
  if (['FONTE_LIMITATA', 'QUOTA_O_POLICY_PROVIDER', 'FONTE_DEGRADATA_SOSPESA'].includes(errore?.codiceGestore)) return 'limitato_temporaneamente';
  return 'errore';
}
...
export async function ordinaProviderPerSalute(db, providers = []) {
  const arricchiti = [];
  for (const provider of providers) arricchiti.push({ provider, salute: await leggiStatoProvider(db, provider.id) });
  return arricchiti.sort((a, b) => {
...
export async function eseguiOperazioneProvider({ db, provider, operazione, configurazione = {}, oraMs = Date.now(), contaRisultati = () => 0, chiaveRichiesta = null }) {
  if (!provider?.id || typeof operazione !== 'function') return { stato: 'provider_non_disponibile', provider: provider?.id || null, saltato: true };

  const regola = regolaFonte(provider.id);
  if (!regola) return {
    stato: 'regole_provider_non_registrate', provider: provider.id, saltato: true,
  ...
  if (statoDichiarato.disponibile === false) {
    ...
    return { stato: statoDichiarato.stato || 'non_disponibile', provider: provider.id, saltato: true };
  }
  const circuito = await puoUsareProvider(db, provider.id, oraMs);
  if (!circuito.consentito) return { stato: 'sospeso_circuit_breaker', provider: provider.id, saltato: true, sospesoFino: circuito.sospesoFino };
  ...
  const cacheMs = Number.isFinite(Number(configurazione.provider_cache_ms)) ? Math.max(0, Number(configurazione.provider_cache_ms)) : null;
  const inizio = Date.now();

  try {
    const gestito = await eseguiConGestoreLimiti({
      provider: provider.id, db, massimoTentativi: tentativiMassimi, chiaveRichiesta, cacheTtlMs: cacheMs,
      operazione: async ({ tentativo, regola: regolaAttiva }) => {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort('timeout_provider'), timeoutMs);
        try { return await operazione({ signal: controller.signal, tentativo, regola: regolaAttiva }); }
        catch (e) {
          if (controller.signal.aborted) {
            const timeout = new Error(`Timeout del provider ${provider.id} dopo ${timeoutMs} ms`);
            timeout.name = 'AbortError'; timeout.status = 408; throw timeout;
          }
          throw e;
    ...
    if (!gestito.daCache && !gestito.richiestaDeduplicata) await registraSuccessoProvider(db, provider.id, { durataMs, risultati });
    return {
      stato: 'ok', provider: provider.id, durataMs, saltato: false, valore: gestito.valore,
      tentativi: gestito.tentativi, attesaLimitiMs: gestito.attesaTotaleMs,
      daCache: Boolean(gestito.daCache), richiestaDeduplicata: Boolean(gestito.richiestaDeduplicata), regolaFonte: gestito.regola
    };
  } catch (e) {
    const durataMs = Date.now() - inizio;
    ...
    const messaggio = timeout ? `Timeout del provider ${provider.id} dopo ${timeoutMs} ms` : String(e?.message || `Errore del provider ${provider.id}`);
    ...
    if (!e?.saltato || e?.codiceGestore === 'QUOTA_FONTE_ESAURITA') {
      await registraErroreProvider(db, provider.id, {
        durataMs, errore: messaggio, codiceErrore: codice,
        sogliaErrori: limitazioneImmediata ? 1 : sogliaErrori,
        sospensioneMinuti: limitazioneImmediata ? sospensioneDaGestoreMinuti : sospensioneMinuti, oraMs
      });
    }
    return {
      stato: statoErroreGestito(e, timeout), provider: provider.id, saltato: Boolean(e?.saltato),
      errore: messaggio, codiceErrore: codice, codiceGestore: e?.codiceGestore || null,
      retryAfterMs, durataMs, regolaFonte: regola
    };
  }
}

export async function eseguiRicercaProvider({ db, provider, richiesta, configurazione = {}, oraMs = Date.now() }) {
  if (!provider || typeof provider.cerca !== 'function') return { stato: 'provider_non_disponibile', provider: provider?.id || null, elementi: [] };
  const esito = await eseguiOperazioneProvider({
    db, provider, configurazione, oraMs,
  ...
  if (esito.stato !== 'ok') return { ...esito, elementi: [] };
  const pagina = esito.valore || {};
  return {
    ...pagina, stato: pagina?.stato || 'ok', provider: provider.id, durataMs: esito.durataMs, saltato: false,
    tentativi: esito.tentativi, attesaLimitiMs: esito.attesaLimitiMs, daCache: esito.daCache,
